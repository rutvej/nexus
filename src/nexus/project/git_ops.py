...
class GitOperations:
    def __init__(self, repo_path: str = None):
        ...
        # 3. Bootstrap workspace: create conftest.py and package __init__.py stubs
        self._bootstrap_workspace()

    def _bootstrap_workspace(self):
        """
        Ensures the workspace has the necessary Python package scaffolding so that
        generated test files can import from the `src` package using paths like
        `from src.models.user import User`.
        Creates:
          - conftest.py at workspace root (adds src/ to sys.path for pytest)
          - src/__init__.py
        ...
        if not conftest.exists():
            conftest.write_text(
                "import sys\nimport os\n"
                "# Allow `from src.X import Y` style imports in tests\n"
                "sys.path.insert(0, os.path.dirname(__file__))\n"
            )

        for pkg_dir in ["src", "src/models", "src/views", "tests"]:
            pkg_path = self.repo_path / pkg_dir
            pkg_path.mkdir(parents=True, exist_ok=True)
            init_file = pkg_path / "__init__.py"
            if not init_file.exists():
                init_file.write_text("# auto-generated by Nexus\n")
